...
// should be correct
layout(local_size_x = WGS1, local_size_y = 1) in;

layout(binding = 0) readonly buffer agm_buf { real agm[]; };
layout(binding = 1) readonly buffer xgm_buf { real xgm[]; };
...
real LoadMatrixA(const int x, const int y,
...
void main() {
  ...
  const int n_tail = consts.n % WGS1;
  const int n_floor = consts.n - n_tail;
  ...
  for (int kwg=0; kwg<n_floor; kwg+=WGS1) {
    ...
    // #pragma unroll
    for (int _w = 0; _w < WPT1; _w += 1) {
      const int gid = _w*int(gl_NumWorkGroups.x*gl_WorkGroupSize.x) + int(gl_GlobalInvocationID.x);
      if (gid < consts.m) {

        // The multiply-add function for the main part (divisable by WGS1)
        if (consts.a_rotated == 0) { // Not rotated
          for (int kloop=0; kloop<WGS1; kloop+=UNROLL1) {
            //#pragma unroll
            for (int _kunroll = 0; _kunroll < UNROLL1; _kunroll += 1) {
              const int k = kwg + kloop + _kunroll;
              real value = LoadMatrixA(gid, k, consts.a_ld, consts.a_offset, consts.parameter, consts.kl, consts.ku);
              if (consts.do_conjugate == 1) { COMPLEX_CONJUGATE(value); }
          ...
          for (int kloop=0; kloop<WGS1; kloop+=UNROLL1) {
            ...
            for (int _kunroll = 0; _kunroll < UNROLL1; _kunroll += 1) {
              const int k = kwg + kloop + _kunroll;
              real value = LoadMatrixA(k, gid, consts.a_ld, consts.a_offset, consts.parameter, consts.kl, consts.ku);
              if (consts.do_conjugate == 1) { COMPLEX_CONJUGATE(value); }
              MultiplyAdd(acc1[_w], xlm[kloop + _kunroll], value);
            }
          }
  ...
  for (int _w = 0; _w < WPT1; _w += 1) {
    ...
    if (gid < consts.m) {
      ...
      if (consts.a_rotated == 0) { // Not rotated
        for (int k=n_floor; k<consts.n; ++k) {
          real value = LoadMatrixA(gid, k, consts.a_ld, consts.a_offset, consts.parameter, consts.kl, consts.ku);
          if (consts.do_conjugate == 1) { COMPLEX_CONJUGATE(value); }
          MultiplyAdd(acc1[_w], xgm[k*consts.x_inc + consts.x_offset], value);
        }
      }
      else { // Transposed
        for (int k=n_floor; k<consts.n; ++k) {
      ...
      AXPBY(ygm[gid*consts.y_inc + consts.y_offset], alpha, acc1[_w], beta, yval);
    }
  }
}
